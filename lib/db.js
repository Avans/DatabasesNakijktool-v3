...
// Aiven ships MySQL with ANSI in sql_mode, which turns on ANSI_QUOTES and
// PIPES_AS_CONCAT. Under those, `WHERE code = "KLS"` reads "KLS" as a column name
// and `a || b` concatenates instead of being OR - so v2 queries that students
// have always written fail with "Unknown column 'KLS'". Pin the session back to
// the MySQL default mode, which matches the 5.7 server v2 ran on.
const STUDENT_SQL_MODE =
  process.env.STUDENT_SQL_MODE ||
...
// Aiven listens on a per-service port, never 3306. Defaulting silently turns a
// forgotten environment variable in Vercel into ECONNREFUSED, which reads like a
// network fault rather than a missing setting - so say what is actually wrong.
function requireEnv() {
  const missing = ['DB_HOST', 'DB_PORT', 'DB_USER', 'DB_PASSWORD'].filter(
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing database configuration: ${missing.join(', ')}. ` +
        'Set these in .env locally, or with `vercel env add <name> production` for a deploy.'
    );
  }
}
...
async function studentQuery(schema, sql) {
  ...
  try {
    ...
    // Pooled connections are reused, so set the mode on every borrow.
    await connection.query('SET SESSION sql_mode = ?', [STUDENT_SQL_MODE]);
